...
class PlayerDefaultUserChoiceGetter:
    def __init__(self, tmdb_type, tmdb_id, season=None, episode=None, **kwargs):
        ...
        self.season = f'{season}' if season else None
        self.episode = f'{episode}' if episode else None
...
class PlayerDefaultUserChoiceSetter(PlayerDefaultUserChoiceGetter):
    ...
    def data(self):
        ...
        sets = True if self.file and self.mode else False
...
def run(*args, **kwargs):
    return PlayerDefaultUserChoiceSetter(*args, **kwargs).update()
