from tmdbhelper.lib.addon.plugin import convert_type, get_localized, ADDONPATH
...
class ListSeasons(ContainerDirectory):
    ...
    def get_special_seasons(self, tmdb_id):
        ...
        if base_item['infolabels'].get('status') not in ('Ended', 'Canceled') and base_item['infoproperties'].get('next_episode_to_air_id'):
